...
/**
 * The HebrewDateFormatter class formats a {@link JDate}.
 * 
 ...
 * </ul>
 * 
 * @see JDate.sourceforge.zmanim.hebrewcalendar.JewishDate
 * @see JCalendar.sourceforge.zmanim.hebrewcalendar.JewishCalendar
 * 
 * @author &copy; Eliyahu Hershfeld 2011
 * @version 0.3
 */
public class JDateFormatter {
	...
	/**
	 * returns if the {@link #formatDayOfWeek(JDate)} will use the long format such as
	...
	/**
	 * Setting to control if the {@link #formatDayOfWeek(JDate)} will use the long format such as
	 * &#x05E8;&#x05D0;&#x05E9;&#x05D5;&#x05DF; or short such as &#x05D0; when formatting the day of week in
	 * {@link #isHebrewFormat() Hebrew}.
	 * 
	 ...
	 * the {@link #formatDayOfWeek(JDate)}
	 * 
	 ...
	 * {@link #formatYomTov(JCalendar)} when formatting the Yom Tov String. The default list of months uses
	 ...
	 * {@link #formatYomTov(JCalendar)} when formatting the Yom Tov String.
	 * 
	 ...
	 * @see #isHebrewFormat()
	 */
	public String formatYomTov(JCalendar jewishCalendar) {
		int index = jewishCalendar.getYomTovIndex();
		if (index == JCalendar.CHANUKAH) {
	...
	public String formatRoshChodesh(JCalendar jewishCalendar) {
		...
		if (jewishCalendar.getJewishDayOfMonth() == 30) {
			if (month < JCalendar.ADAR || (month == JCalendar.ADAR && jewishCalendar.isJewishLeapYear())) {
				month++;
			} else { // roll to Nissan
				month = JCalendar.NISSAN;
			}
		}

		// This method is only about formatting, so we shouldn't make any changes to the params passed in...
		jewishCalendar = (JCalendar) jewishCalendar.clone();
		jewishCalendar.setJewishMonth(month);
		formattedRoshChodesh = hebrewFormat ? hebrewHolidays[JCalendar.ROSH_CHODESH]
				: transliteratedHolidays[JCalendar.ROSH_CHODESH];
	 ...
	 * @see #format(JDate)
	 * @see #formatDayOfWeek(JDate)
	 * @see #formatMonth(JDate)
	 * @see #formatOmer(JCalendar)
	 * @see #formatParsha(JCalendar)
	 * @see #formatYomTov(JCalendar)
	 */
	...
	public String formatDayOfWeek(JDate jewishDate) {
		if (hebrewFormat) {
	 ...
	 */
	public String formatParsha(JCalendar jewishCalendar) {
	 ...
	 *         "21 Shevat, 5729" if not.
	 */
	public String format(JDate jewishDate) {
		if (isHebrewFormat()) {
	 ...
	 * @see #setTransliteratedMonthList(String[])
	 */
	public String formatMonth(JDate jewishDate) {
		final int month = jewishDate.getJewishMonth();
		if (isHebrewFormat()) {
			if (jewishDate.isJewishLeapYear() && month == JDate.ADAR) {
				return hebrewMonths[13] + (useGershGershayim ? GERESH : ""); // return Adar I, not Adar in a leap year
			} else if (jewishDate.isJewishLeapYear() && month == JDate.ADAR_II) {
	 ...
	 * @see #setHebrewOmerPrefix(String)
	 */
	public String formatOmer(JCalendar jewishCalendar) {
	 ...
	 * Rosh Hashana, the second letter represents the lengths of Cheshvan and Kislev ({@link JDate#SHELAIMIM
	 * Shelaimim} , {@link JDate#KESIDRAN Kesidran} or {@link JDate#CHASERIM Chaserim}) and the 3rd letter
	...
	public String getFormattedKviah(int jewishYear) {
		JDate jewishDate = new JDate(jewishYear, JDate.TISHREI, 1); // set date to Rosh Hashana
		...
		returnValue += (kviah == JDate.CHASERIM ? "\u05D7" : kviah == JDate.SHELAIMIM ? "\u05E9" : "\u05DB");
		jewishDate.setJewishDate(jewishYear, JDate.NISSAN, 15); // set to Pesach of the given year
